...
```text
Share (URL):
  Editor ──Copy Link (RLLSketch)──► viewer.php?data=… ──► Viewer
  Viewer ──Edit (RLLSketch URL)──► Editor

LLM loop:
  Viewer ──Copy LLSketch──┐
                          ├──► LLM Chat ──Paste LLSketch──► Editor
  Editor ──Copy LLSketch──┘         ▲                           │
                                    └──── Copy LLSketch ────────┘
```
